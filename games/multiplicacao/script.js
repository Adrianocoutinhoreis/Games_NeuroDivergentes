/* ============================================================
   ...
   Modelo pedagógico da multiplicação: "grupos iguais". A conta a × b
   aparece como "a" grupos, cada um com "b" desenhos iguais; a criança
   conta todos os desenhos para achar o total. É a representação
   concreta da multiplicação (parcelas repetidas / arranjo).
...
(function () {
    ...
    // Multiplicação cresce rápido: mantemos poucos grupos e poucos itens
    // por grupo, para o total desenhado ficar sempre confortável de contar.
    var NIVEIS = {
        1: { maxGrupos: 3, maxPorGrupo: 3 },   // até 3×3 = 9
        2: { maxGrupos: 4, maxPorGrupo: 5 },   // até 4×5 = 20
        3: { maxGrupos: 5, maxPorGrupo: 5 }    // até 5×5 = 25
    };

    // Nível de dificuldade escolhido pelo professor (cognitivo.nivelJogo)
    var NIVEL_POR_DIFICULDADE = { facil: 1, medio: 2, dificil: 3 };

    // Um desenho por pergunta (sorteado), o mesmo em todos os grupos
    var DESENHOS = ["🥕", "🍎", "⭐", "🌸", "🍓", "🐤", "🌷", "🍊"];
    ...
    var BALAO_PADRAO = "Quantos ao todo?";
    ...
    function aplicarAdaptacoesVisuais(config) {
        ...
        // "padrao" preserva a identidade própria do jogo (aqui, rosa).
    ...
    var el = {
        ...
        gruposMulti: document.getElementById("grupos-multi"),
    ...
    function gerarPergunta() {
        var nivel = NIVEIS[estado.nivel];

        // "a" grupos de "b" desenhos (ambos de pelo menos 2, para ser
        // sempre uma multiplicação de verdade e com o que contar)
        estado.a = sortear(2, nivel.maxGrupos);
        estado.b = sortear(2, nivel.maxPorGrupo);
        ...
        desenharGrupos(desenho, estado.a, estado.b);
    ...
    // Desenha "numGrupos" grupos, cada um com "porGrupo" desenhos iguais.
    // A entrada dos desenhos é levemente escalonada (fade suave), do
    // primeiro ao último grupo, para não aparecer tudo de uma vez.
    function desenharGrupos(desenho, numGrupos, porGrupo) {
        el.gruposMulti.innerHTML = "";
        var indiceGlobal = 0;
        for (var g = 0; g < numGrupos; g++) {
            var grupo = document.createElement("div");
            grupo.className = "grupo-itens";
            for (var i = 0; i < porGrupo; i++) {
                var item = document.createElement("span");
                item.className = "item-visual";
                item.textContent = desenho;
                item.style.animationDelay = (indiceGlobal * 0.05) + "s";
                grupo.appendChild(item);
                indiceGlobal++;
            }
            el.gruposMulti.appendChild(grupo);
        }
    }
